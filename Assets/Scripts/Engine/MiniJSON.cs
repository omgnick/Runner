...
public class MiniJSON
{
	...
	private const int TOKEN_INT = 8;
	private const int TOKEN_FLOAT = 9;
	private const int TOKEN_TRUE = 10;
	private const int TOKEN_FALSE = 11;
	private const int TOKEN_NULL = 12;
	...
	protected static object parseValue( char[] json, ref int index, ref bool success )
	{
		switch( lookAhead( json, index ) )
		{
			...
			case MiniJSON.TOKEN_INT:
				return parseInt( json, ref index );
			case MiniJSON.TOKEN_FLOAT:
				return parseFloat( json, ref index );
...
s += Char.ConvertFromUtf32((int)codePoint);
	...
	protected static float parseFloat( char[] json, ref int index )
	{
		eatWhitespace( json, ref index );
	...
	protected static object parseInt( char[] json, ref int index )
	{
		eatWhitespace( json, ref index );
		...
		Array.Copy( json, index, numberCharArray, 0, charLength );
		index = lastIndex + 1;
		//UnityEngine.Debug.Log(new string( numberCharArray ));
		string s = new string( numberCharArray );
		//return int.Parse( new string( numberCharArray ) ); // , CultureInfo.InvariantCulture);
		int int_result;
		if(int.TryParse(s, out int_result)) {
			return int_result;
		} else {
			//RtsDebug.Log(s);
			//Number
			//RtsDebug.Log(float.Parse( s,  ).ToString("0.0000"));
			float float_result = float.Parse(s);
			if(float_result.ToString("R") == s) {
				return float_result;
			} else {
				//RtsDebug.Log(double.Parse( s ).ToString("0.0000"));
